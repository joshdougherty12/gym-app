...
import { cautionsFor, describe, EXERCISE_LIBRARY } from '../data/exercises'
...
function withDescription(e: Exercise): Exercise {
  const d = describe(e)
  const c = cautionsFor(e)
  return { ...e, ...(d && !e.description ? { description: d } : {}), ...(c.length && !e.cautions ? { cautions: c } : {}) }
}
